  ...
  var dTree = {
    ...
    _preprocess: function _preprocess(data, opts) {
      ...
      var reconstructTree = function reconstructTree(person, parent) {
        ...
        node.class += person['gender'] ? ' '+person['gender'] : '';
        
        if(typeof node.extra === 'undefined'){
          node.extra = {};
        }

        node.extra.gender = person['gender'];
        node.extra.nickname = person['nickname'];
        ...
        _.forEach(person.marriages, function (marriage, index) {
          ...
          spouse.class += sp['gender'] ? ' '+sp['gender'] : '';

          if(typeof spouse.extra === 'undefined'){
            spouse.extra = {};
          }

          spouse.extra.gender = sp['gender'];
          spouse.extra.nickname = sp['nickname'];
